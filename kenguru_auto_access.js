// KENGURU_SELF_REPORT_10S_V1. The server, not the browser, grants access.
...
function kenguruAutoSetText(){
  Object.assign(T.kk, {
    ...
    howText:'Оқушы сыныбы мен тілін таңдайды. «Төлем жасадым» батырмасы басылғаннан кейін 10 секундтан соң 30 минуттық бір мүмкіндік ашылады.',
    waitTitle:'Тестке рұқсат',
    waitText:'«Төлем жасадым» батырмасы басылғаннан кейін тестке рұқсат 10 секундтан соң автоматты түрде ашылады.'
  });
  Object.assign(T.ru, {
    iPaid:'Я оплатил(а)',
    payText:'После оплаты через Kaspi нажмите «Я оплатил(а)». Доступ к тесту откроется через 10 секунд.',
    howText:'Ученик выбирает класс и язык. Через 10 секунд после нажатия «Я оплатил(а)» открывается одна попытка на 30 минут.',
    waitTitle:'Доступ к тесту',
    waitText:'После нажатия «Я оплатил(а)» доступ к тесту автоматически откроется через 10 секунд.'
  });
}
...
async function kenguruAutoApply(s){
  ...
  if(s.auto_access_pending){
    showOnly('waitCard');kenguruAutoWaitButton(false);kenguruAutoRefreshText();
